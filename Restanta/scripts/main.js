(function () {
  ...
  socket.on('user options', setUserOptions)
  ...
  function onSuccessfullySaved() {
    ...
    for (var i = 0; i < selectedCategories.length; i++) {
      myCategsHtml += htmlProvider.toCategoryLinkHTML(selectedCategories[i].name)
    }
    cList.innerHTML = myCategsHtml
    selectedCategTitle.innerText = selectCategory.name
  ...
  function setUserOptions(data) {
    ...
    mainBody.style.background = 'url(./images/' + data.theme +'.jpg)';

    if (data.theme == "bgr2" || data.theme == "bgr5" ) {
  ...
  function startTimer() {
//    timerInterval = setInterval(() => loadArticles(selectCategory), parseInt(refreshTime) * 60 * 1000)
  }

  function resetBackground() {
    var mainBody = document.getElementsByClassName('mainBody')[0]
    mainBody.style.background = 'url(./images/bgr1.jpg)';
    mainBody.style.color = '#fff';
  }
