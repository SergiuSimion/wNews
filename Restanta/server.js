...
MongoClient.connect(dbURL, function (err, client) {
  ...
  io.on('connection', function (connection) {
      ...
      wNewsDb.collection('login', function (err, collection) {
        collection.find({
          ...
          if (entries.length > 0) {
            ...
            wNewsDb.collection('options').find({ userID: uID }).toArray(function (err, options) {
              if (options.length > 0) {
                connection.emit('user options', options[0]);
              }
            })
    ...
    connection.on('save selected categories', (data) => {
      ...
      wNewsDb.collection('categories').remove({userID: userID }, function() {
        if (data[0].name) {
    ...
    connection.on('get options', (data) => {
      const options = {}
      wNewsDb.collection('options').find({ userID: data }).toArray(function (err, entries) {
        if (entries[0]) {
          options.theme = entries[0].theme
          options.refreshTime = entries[0].refreshTime
        } else {
          options.theme = ""
          options.refreshTime = 1
        }

        wNewsDb.collection('categories').find({ userID: data }).toArray(function (err, entries) {
          options.categories = entries
          connection.emit('user options', options)
        })
      })
    })

    connection.on('save options', (data) => {
      const option = models.createOption(data.theme, data.refreshTime, data.userID)
      wNewsDb.collection('options').remove({userID: data.userID }, function() {
        wNewsDb.collection('options').save(option, {w: 1}, function(err, records){});
      })
